...
// QueryFromgender chains the current query on the fromgender edge.
func (gq *GenderQuery) QueryFromgender() *PatientQuery {
...
//  WithFromgender tells the query-builder to eager-loads the nodes that are connected to
// the "fromgender" edge. The optional arguments used to configure the query builder of the edge.
func (gq *GenderQuery) WithFromgender(opts ...func(*PatientQuery)) *GenderQuery {
	query := &PatientQuery{config: gq.config}
	for _, opt := range opts {
		opt(query)
	}
	gq.withFromgender = query
	return gq
}
...
func (gq *GenderQuery) sqlAll(ctx context.Context) ([]*Gender, error) {
	...
	if query := gq.withFromgender; query != nil {
		...
		query.Where(predicate.Patient(func(s *sql.Selector) {
			s.Where(sql.InValues(gender.FromgenderColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			...
			node.Edges.Fromgender = append(node.Edges.Fromgender, n)
		}
	}
