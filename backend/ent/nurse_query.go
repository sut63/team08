...
// QueryFromnurse chains the current query on the fromnurse edge.
func (nq *NurseQuery) QueryFromnurse() *RentQuery {
	query := &RentQuery{config: nq.config}
...
//  WithFromnurse tells the query-builder to eager-loads the nodes that are connected to
// the "fromnurse" edge. The optional arguments used to configure the query builder of the edge.
func (nq *NurseQuery) WithFromnurse(opts ...func(*RentQuery)) *NurseQuery {
	query := &RentQuery{config: nq.config}
	for _, opt := range opts {
		opt(query)
	}
	nq.withFromnurse = query
	return nq
}
...
func (nq *NurseQuery) sqlAll(ctx context.Context) ([]*Nurse, error) {
	...
	if query := nq.withFromnurse; query != nil {
		...
		query.Where(predicate.Rent(func(s *sql.Selector) {
			s.Where(sql.InValues(nurse.FromnurseColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			...
			node.Edges.Fromnurse = append(node.Edges.Fromnurse, n)
		}
	}
