...
export default function ComponentsTable() {
    ...
    const [prescription, getPrescription] = useState<EntPrescription>();
    ...
    const [prescriptions, setPrescriptions] = React.useState<EntPrescription[]>([]);

    const getPrescriptions = async () => {
        const res = await http.listPrescription({ limit: 10, offset: 0 });
        setPrescriptions(res);
    };
    const searchPrescription = async () => {
        const res = await http.getPrescription({ id: prescription_id })
        if (res != undefined) {
            getPrescription(res);
        }
    };
              ...
              {search ? (
                ...
                <TableBody>
                    <TableRow >
                        ...
                        <TableCell align="center">{prescription?.prescripNumber}</TableCell>
                        <TableCell align="center">{prescription?.edges?.patient?.patientName}</TableCell>
                        <TableCell align="center">{prescription?.edges?.doctor?.doctorName}</TableCell>
                        <TableCell align="center">{prescription?.edges?.drug?.drugName}</TableCell>
                        <TableCell align="center">{moment(prescription?.prescripDateTime).format('DD/MM/YYYY hh:mm')}</TableCell>
                        <TableCell align="center">{prescription?.edges?.nurse?.nurseName}</TableCell>
                        <TableCell align="center">{prescription?.prescripIssue}</TableCell>
                        <TableCell align="center">{prescription?.prescripNote}</TableCell>
                    </TableRow>
                </TableBody>
                </Table>
                    </TableContainer>
