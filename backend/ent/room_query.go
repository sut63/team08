...
// QueryRents chains the current query on the rents edge.
func (rq *RoomQuery) QueryRents() *RentQuery {
...
//  WithRents tells the query-builder to eager-loads the nodes that are connected to
// the "rents" edge. The optional arguments used to configure the query builder of the edge.
func (rq *RoomQuery) WithRents(opts ...func(*RentQuery)) *RoomQuery {
	query := &RentQuery{config: rq.config}
...
func (rq *RoomQuery) sqlAll(ctx context.Context) ([]*Room, error) {
	...
	if query := rq.withRents; query != nil {
		...
		query.Where(predicate.Rent(func(s *sql.Selector) {
			s.Where(sql.InValues(room.RentsColumn, fks...))
		}))
	...
	if query := rq.withRoomtype; query != nil {
		...
		nodeids := make(map[int][]*Room)
		for i := range nodes {
			if fk := nodes[i].roomtype_id; fk != nil {
				ids = append(ids, *fk)
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "roomtype_id" returned %v`, n.ID)
			}
			for i := range nodes {
