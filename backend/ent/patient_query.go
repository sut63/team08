...
func (pq *PatientQuery) QueryFrompatient() *RentQuery {
	query := &RentQuery{config: pq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := pq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (pq *PatientQuery) sqlAll(ctx context.Context) ([]*Patient, error) {
	...
	if query := pq.withFrompatient; query != nil {
		...
		query.Where(predicate.Rent(func(s *sql.Selector) {
			s.Where(sql.InValues(patient.FrompatientColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
